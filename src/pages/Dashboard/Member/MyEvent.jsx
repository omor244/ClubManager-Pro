...
const MyEvent = () => {
    ...
    const { data: events = [], isLoading, refetch } = useQuery({
        ...
        queryFn: async () => {
            const res = await axiosSicure.get(`/register/events/${user?.email}`)
            return res.data;
        }
    });

    const handellivefromevent =  (id) => {
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                ...
                const res = await axiosSicure.delete(`/events/delete/${id}`)
                        ...
                        events.map(event => (
                            ...
                            <tr key={event._id} className="block md:table-row border-b md:border-none mt-4 mb-3 md:mb-0 p-2 md:p-0">
                                ...
                                <td className="block md:table-cell pb-3 md:pb-0 text-right">
                                    ...
                                    <button onClick={() => handellivefromevent(event._id)} className="btn btn-ghost btn-md btn-outline text-info hover:bg-info hover:text-white">
                                        Leave
                                    </button>
                                </td>
                            </tr>
